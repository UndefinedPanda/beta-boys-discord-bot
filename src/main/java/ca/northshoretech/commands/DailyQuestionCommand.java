...
public class DailyQuestionCommand extends ListenerAdapter {
    ...
    private final String botCommandsChannelId = BetaBoys.getConfig().get("COMMANDS_CHANNEL");
    private final String dailyQuestionChannelId = BetaBoys.getConfig().get("DAILY_QUESTION_CHANNEL");
    private final String adminRoleId = BetaBoys.getConfig().get("ADMIN_ROLE_ID");
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        if (!event.getChannel().getId().equals(botCommandsChannelId))
            return;
        ...
        if (adminRole == null) {
            BetaBoys.getLogger().error("No admin role found. Please make sure theres an admin role in the server");
            return;
        }
        List<Role> authorRoles = Objects.requireNonNull(event.getMember()).getRoles();
        if (!authorRoles.contains(adminRole)) {
            EmbedHelper.sendWarningEmbed(event.getChannel(), "You do not have the required admin permissions to use this command.");
            return;
        }

        // get the question from the message being sent
        String question = event.getMessage().getContentRaw().substring(commandPrefix.length());
        ...
        TextChannel dailyQuestionChannel = event.getGuild().getTextChannelById(dailyQuestionChannelId);
        if (dailyQuestionChannel == null) {
            BetaBoys.getLogger().error("There was an error finding the daily question channel by its ID");
            EmbedHelper.sendErrorEmbed(event.getChannel(), "There was an error finding the daily question channel by its ID.");
            return;
        }
        // send message to the daily-question channel
        dailyQuestionChannel.sendMessage("@everyone").setEmbeds(embedBuilder.build()).queue();
    }
}
