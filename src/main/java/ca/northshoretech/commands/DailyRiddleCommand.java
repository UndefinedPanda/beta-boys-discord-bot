...
public class DailyRiddleCommand extends ListenerAdapter {
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        String[] riddleMessage = event.getMessage().getContentRaw().substring(commandPrefix.length()).split("-");
        if (riddleMessage.length < 2) {
            BetaBoys.getLogger().error("The riddle message must have a riddle and an answer");
        ...
        if (dailyRiddleChannel == null) {
            BetaBoys.getLogger().error("There was an error finding the daily riddle channel by its ID");
